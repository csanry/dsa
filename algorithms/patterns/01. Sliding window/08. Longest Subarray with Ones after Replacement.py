...
def length_of_longest_substring(arr, k):
    ...
    for window_end in range(len(arr)):
        ...
        # repeated max_ones_count times, this means we can have a window with max_ones_count 1s
        # and the remaining are 0s which must be replaced with 1s
        # if the remaining 1s are more than 'k', it is the time to shrink the window as we
        # can't replace more than 'k' 0s
